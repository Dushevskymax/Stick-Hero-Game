    ...
    execute: function () {
      ...
      _export("ColumnManager", ColumnManager = class ColumnManager {
        constructor(columnWidth, columnHeight) {
          ...
          this.columnWidth = 0;
          this.columnHeight = 0;
          var canvasSize = (_crd && SceneUtils === void 0 ? (_reportPossibleCrUseOfSceneUtils({
            error: Error()
        ...
        generateNextColumn() {
          var minWidth = this.columnWidth / 3;
          var maxWidth = this.columnWidth * 2;
          var randomWidth = (_crd && SceneUtils === void 0 ? (_reportPossibleCrUseOfSceneUtils({
            error: Error()
          }), SceneUtils) : SceneUtils).randomRange(minWidth, maxWidth);
          var minPosition = this.canvasWidth / 2 + 1.5 * this.columnWidth;
          var maxPosition = 1.5 * this.canvasWidth - randomWidth / 2;
          var randomPosition = (_crd && SceneUtils === void 0 ? (_reportPossibleCrUseOfSceneUtils({
            error: Error()
        ...
        calculateInitialPositions(startColumnWidth, playerWidth) {
          var startColumnX = -this.canvasWidth / 2 + startColumnWidth / 2;
          var playerX = startColumnX + playerWidth / 4;
          var nextColumnX = this.generateNextColumn().x - this.canvasWidth;
          return {
            startColumnX,
            playerX,
            nextColumnX
