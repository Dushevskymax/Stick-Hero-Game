    ...
    execute: function () {
      ...
      _export("StickController", StickController = (_dec = ccclass('StickController'), _dec2 = property(_crd && Entities === void 0 ? (_reportPossibleCrUseOfEntities({
        ...
        createStick() {
          ...
          var columnWidth = startColumn.getComponent(UITransform).width;
          var columnHeight = startColumn.getComponent(UITransform).height;
          var columnRightX = startColumn.position.x + columnWidth / 2;
          var columnTopY = startColumn.position.y + columnHeight / 2;
          this.currentStick.setPosition(columnRightX, columnTopY, 0);
          var canvas = startColumn.parent;
        ...
        growStick(deltaTime) {
          if (this.isGrowing && this.currentStick) {
            var currentScale = this.currentStick.getScale();
            var stickTransform = this.currentStick.getComponent(UITransform);
            var newY = currentScale.y + this.growthSpeed * deltaTime;
            var maxScaleY = this.maxHeight / stickTransform.height;
