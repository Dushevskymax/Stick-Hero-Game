    ...
    execute: function () {
      ...
      _export("StickController", StickController = class StickController {
        ...
        startGrowing() {
          ...
          this.currentScaleY = 0;
          var startColumnInfo = this.view.getStartColumnInfo();
          var startX = startColumnInfo.x + startColumnInfo.width / 2;
          var startY = startColumnInfo.height / 2;
        ...
        update(deltaTime) {
          ...
          var stickInfo = this.view.getStickInfo();
          var maxScaleY = this.view.getMaxHeight() / stickInfo.height;
        ...
        checkResult() {
          try {
            var startColumnInfo = this.view.getStartColumnInfo();
            var nextColumnInfo = this.view.getNextColumnInfo();
            var stickInfo = this.view.getStickInfo();
            var startRightX = startColumnInfo.x + startColumnInfo.width / 2;
            var stickLength = stickInfo.height * stickInfo.scaleY;
            var stickEndX = startRightX + stickLength;
            var nextLeftX = nextColumnInfo.x - nextColumnInfo.width / 2;
            var nextRightX = nextColumnInfo.x + nextColumnInfo.width / 2;
            ...
            if (stickEndX >= nextLeftX && stickEndX <= nextRightX) {
              var playerWidth = this.view.getPlayerWidth();
              var playerX = nextColumnInfo.x + playerWidth / 4;
